...
def get_parser():
    ...
    parser.add_argument('--data_dir', type=str, default='./dataset/val', help='model load path')
    parser.add_argument('--pair_path', type=str, default='./image_pair.txt', help='model load path')
    parser.add_argument('--lfw', type=bool, default=False, help='model load path')
...
def demo_gray():
    testDataset = ValImageDataset(args['data_dir'], args)
    ...
    PSNR = []
    with torch.no_grad():
        for i, (image, gt, _, basename) in tqdm(enumerate(testDataLoader), total=len(testDataLoader)):
...
def demo_color():
    testDataset = ColorImageDataset('./dataset/val', args)
    testDataLoader = torch.utils.data.DataLoader(
        dataset=testDataset,
        batch_size=1,
        shuffle=False,
